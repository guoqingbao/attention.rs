 ...
 *
 * Uses CUTLASS GemmUniversal with kGrouped mode and PtrArray-based dispatching,
 * following the FlashInfer/SGLang approach for FP4 MoE.
 */
...
__global__ void setup_moe_group_gemm_args(
    ...
    ElementAB* b_base,
    ElementC* out_base,
    ElementSF* a_scales_base,
    ElementSF* b_scales_base,
    ElementAccumulator* alphas_base,
    ...
    cudaStream_t stream)
{
  ...
  auto a_ptrs = static_cast<ElementType**>(alloc(ptr_array_bytes));
  auto b_ptrs = static_cast<ElementType**>(alloc(ptr_array_bytes));
  auto out_ptrs = static_cast<ElementD**>(alloc(ptr_array_bytes));
  auto a_sf_ptrs = static_cast<ElementSFType**>(alloc(ptr_array_bytes));
  auto b_sf_ptrs = static_cast<ElementSFType**>(alloc(ptr_array_bytes));
  auto alpha_ptrs = static_cast<float**>(alloc(ptr_array_bytes));
  ...
  // Launch setup kernel to fill pointer arrays, strides, and scale layouts
  setup_moe_group_gemm_args<
      ...
      <<<1, num_experts, 0, stream>>>(
          ...
          const_cast<ElementType*>(static_cast<const ElementType*>(a)),
          const_cast<ElementType*>(static_cast<const ElementType*>(b)),
          static_cast<ElementD*>(output),
          const_cast<ElementSFType*>(static_cast<const ElementSFType*>(a_blockscale)),
          const_cast<ElementSFType*>(static_cast<const ElementSFType*>(b_blockscales)),
  ...
  typename Gemm::GemmKernel::MainloopArguments mainloop_args{
      reinterpret_cast<const ElementType**>(a_ptrs),
      a_strides,
      reinterpret_cast<const ElementType**>(b_ptrs),
      b_strides,
      reinterpret_cast<const ElementSFType**>(a_sf_ptrs),
      layout_sfa_arr,
      reinterpret_cast<const ElementSFType**>(b_sf_ptrs),
      layout_sfb_arr
  };
  ...
  fusion_args.alpha_ptr_array = reinterpret_cast<float**>(alpha_ptrs);
