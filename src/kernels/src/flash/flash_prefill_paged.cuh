// Native Flash Attention v2 — Paged Prefill (BF16 KV cache), SM80+.
//
// Adapted from atlas inferspark kernels for high-bandwidth GPUs (A100/H100/H800/B200).
...
//
// Warp roles:
//   QK^T: warps 0-1, each owns 16 M-rows (0-15, 16-31)
    ...
    for (unsigned int kv_block = 0; kv_block < num_kv_blocks; kv_block++) {
        ...
        if (warp_id < 2) {
            ...
            for (unsigned int ks = 0; ks < (HDIM / 16); ks++) {
                ...
                // Load Q fragments via ldmatrix on SM80+
                unsigned int sa_q0 = __cvta_generic_to_shared(
                    &sQ[(ar0 * HDIM_PAD + ac0)]);
                unsigned int sa_q1 = __cvta_generic_to_shared(
                    &sQ[(ar1 * HDIM_PAD + ac0)]);
                unsigned int sa_q2 = __cvta_generic_to_shared(
                    &sQ[(ar0 * HDIM_PAD + ac1)]);
                unsigned int sa_q3 = __cvta_generic_to_shared(
                    &sQ[(ar1 * HDIM_PAD + ac1)]);
...
extern "C" __global__ void flash_prefill_paged(
        ...
        for (unsigned int _i = (t); _i < TILE_CHUNKS_512; _i += (stride)) { \
            ...
            if (_pos < (kv_l)) { \
                ...
                const void* _gm = (const void*)( \
                    (cache) + (unsigned long long)_pb * _ps + (unsigned long long)_bo * _rs \
                    + (unsigned long long)(kvh) * head_dim + _col); \
                asm volatile("cp.async.cg.shared.global [%0], [%1], 16;" :: "r"(_sa), "l"(_gm)); \
    ...
    // Load Q + K[0]
    {
        ...
        if (num_kv_blocks > 0) {
            LOAD_KV_512(K_cache, block_table, smem_K, 0, kv_len, kv_head, tid, NUM_THREADS_512);
        }
        asm volatile("cp.async.commit_group;");
        asm volatile("cp.async.wait_group 0;");
    }
