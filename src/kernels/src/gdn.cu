...
// =============================================================================
// Chunked Gated Delta Rule (Decode)
// =============================================================================

template <typename T>
__global__ void chunk_gated_delta_rule_decode_kernel(
    const T* __restrict__ q,   // [batch, heads, dim] (seq_len=1 flattened)
    const T* __restrict__ k,   // [batch, heads, dim]
    const T* __restrict__ v,   // [batch, heads, dim]
    const T* __restrict__ g,   // [batch, heads]
    const T* __restrict__ beta, // [batch, heads]
    T* __restrict__ state,      // [batch, heads, dim, dim]
    T* __restrict__ out,        // [batch, heads, dim]
    int batch_size,
    int num_heads,
    int head_dim) {
    int b_idx = blockIdx.x;
    int h_idx = blockIdx.y;
    if (b_idx >= batch_size || h_idx >= num_heads) {
        return;
    }

    int head_offset = b_idx * num_heads + h_idx;
    int vec_offset = head_offset * head_dim;
    int state_offset = head_offset * head_dim * head_dim;

    const T* q_vec = q + vec_offset;
    const T* k_vec = k + vec_offset;
    const T* v_vec = v + vec_offset;

    float g_val = to_float(g[head_offset]);
    float beta_val = to_float(beta[head_offset]);

    T* state_matrix = state + state_offset;
    T* out_vec = out + vec_offset;

    int tid = threadIdx.x;
    int stride = blockDim.x;

    for (int idx = tid; idx < head_dim * head_dim; idx += stride) {
        int row = idx / head_dim;
        int col = idx % head_dim;
        float old_s = to_float(state_matrix[idx]);
        float upd = beta_val * to_float(k_vec[row]) * to_float(v_vec[col]);
        state_matrix[idx] = from_float<T>(g_val * old_s + upd);
    }

    __syncthreads();

    for (int col = tid; col < head_dim; col += stride) {
        float acc = 0.0f;
        for (int row = 0; row < head_dim; ++row) {
            acc += to_float(q_vec[row]) *
                   to_float(state_matrix[row * head_dim + col]);
        }
        out_vec[col] = from_float<T>(acc);
    }
}

template <typename T>
void launch_chunk_gated_delta_rule_decode(const T* q, const T* k, const T* v,
                                          const T* g, const T* beta, T* state,
                                          T* out, int batch, int heads, int dim,
                                          cudaStream_t stream) {
    dim3 grid(batch, heads);
    int threads = 256;
    chunk_gated_delta_rule_decode_kernel<<<grid, threads, 0, stream>>>(
        q, k, v, g, beta, state, out, batch, heads, dim);
    CHECK_CUDA(cudaGetLastError());
}

extern "C" void chunk_gated_delta_rule_decode_f32(
    const float* q, const float* k, const float* v, const float* g,
    const float* beta, float* state, float* out, int batch, int heads, int dim,
    cudaStream_t stream) {
    launch_chunk_gated_delta_rule_decode(q, k, v, g, beta, state, out, batch,
                                         heads, dim, stream);
}

extern "C" void chunk_gated_delta_rule_decode_f16(
    const half* q, const half* k, const half* v, const half* g,
    const half* beta, half* state, half* out, int batch, int heads, int dim,
    cudaStream_t stream) {
    launch_chunk_gated_delta_rule_decode(q, k, v, g, beta, state, out, batch,
                                         heads, dim, stream);
}
...
extern "C" void chunk_gated_delta_rule_decode_bf16(
    ...
    const __nv_bfloat16* g, const __nv_bfloat16* beta,
    __nv_bfloat16* state, __nv_bfloat16* out, int batch, int heads, int dim,
    cudaStream_t stream) {
    launch_chunk_gated_delta_rule_decode(q, k, v, g, beta, state, out, batch,
                                         heads, dim, stream);
}
...
template <typename T>
__global__ void chunk_gated_delta_rule_prefill_kernel(
    const T* __restrict__ q,    // [batch, seq_len, heads, dim]
    const T* __restrict__ k,    // [batch, seq_len, heads, dim]
    const T* __restrict__ v,    // [batch, seq_len, heads, dim]
    const T* __restrict__ g,    // [batch, seq_len, heads]
    const T* __restrict__ beta, // [batch, seq_len, heads]
    T* __restrict__ state,      // [batch, heads, dim, dim]
    T* __restrict__ out,        // [batch, seq_len, heads, dim]
    int batch_size,
    int seq_len,
    int num_heads,
    int head_dim) {
    ...
    int bh_offset = b_idx * num_heads + h_idx;
    int state_offset = bh_offset * head_dim * head_dim;
    T* state_matrix = state + state_offset;
    ...
    for (int t = 0; t < seq_len; ++t) {
        int g_off = (b_idx * seq_len + t) * num_heads + h_idx;
        int vec_off = ((b_idx * seq_len + t) * num_heads + h_idx) * head_dim;

        float g_val = to_float(g[g_off]);
        float beta_val = to_float(beta[g_off]);

        const T* q_vec = q + vec_off;
        const T* k_vec = k + vec_off;
        const T* v_vec = v + vec_off;
        T* out_vec = out + vec_off;
...
template <typename T>
void launch_chunk_gated_delta_rule_prefill(const T* q, const T* k, const T* v,
                                           const T* g, const T* beta,
                                           T* state, T* out, int batch,
                                           int seq_len, int heads, int dim,
                                           cudaStream_t stream) {
    dim3 grid(batch, heads);
    int threads = 256;
    chunk_gated_delta_rule_prefill_kernel<<<grid, threads, 0, stream>>>(
        q, k, v, g, beta, state, out, batch, seq_len, heads, dim);
    CHECK_CUDA(cudaGetLastError());
}

extern "C" void chunk_gated_delta_rule_prefill_f32(
    const float* q, const float* k, const float* v, const float* g,
    const float* beta, float* state, float* out, int batch, int seq_len,
    int heads, int dim, cudaStream_t stream) {
    launch_chunk_gated_delta_rule_prefill(q, k, v, g, beta, state, out, batch,
                                          seq_len, heads, dim, stream);
}

extern "C" void chunk_gated_delta_rule_prefill_f16(
    const half* q, const half* k, const half* v, const half* g,
    const half* beta, half* state, half* out, int batch, int seq_len,
    int heads, int dim, cudaStream_t stream) {
    launch_chunk_gated_delta_rule_prefill(q, k, v, g, beta, state, out, batch,
                                          seq_len, heads, dim, stream);
}

extern "C" void chunk_gated_delta_rule_prefill_bf16(
    const __nv_bfloat16* q, const __nv_bfloat16* k, const __nv_bfloat16* v,
    const __nv_bfloat16* g, const __nv_bfloat16* beta,
    __nv_bfloat16* state, __nv_bfloat16* out, int batch, int seq_len,
    int heads, int dim, cudaStream_t stream) {
    launch_chunk_gated_delta_rule_prefill(q, k, v, g, beta, state, out, batch,
                                          seq_len, heads, dim, stream);
}
